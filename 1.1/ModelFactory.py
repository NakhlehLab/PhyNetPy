...
from PhyNetPy.NetworkParser import NetworkParser as np
...
class ANetworkNode(NetworkNode, CalculationNode):
    ...
    def node_move_bounds(self) -> list[int]:
        ...
        The first value in the return list is the amount that a node can be moved
        toward the root/parent node. The second value is the amount that a node
        can be moved toward the child node.

        Args:
            N/A
        Returns:
            list[int]: A list of two integers, the parental and child based 
                       bounds for a node move.
        """
        
        parents : list[NetworkNode] = self.get_parent(return_all = True)
        children = self.get_children()
        
        if parents is not None:
            #max, since closest parent is the one farthest from root/t=0
            if len(parents) != 1:
        ...
        if children is not None:
            #min, since closest child is the one closest to root/t=0
...
class AccumulatorComponent(ModelComponent):
    ...
    def __init__(self, 
        ...
        Args:
            ...
            attach_types (list[type]): The types of model node that this 
                                       accumulator should be attached to.
        Returns:
            N/A
        """
...
class InformationComponent(ModelComponent):
    ...
    def __init__(self, 
        ...
        Args:
            dependencies (set[type]): List of component dependencies
            info (Any): Any relevant information that a node should have access to.
            attach_types (list[type]): The types of model node that this 
