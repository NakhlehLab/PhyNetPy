...
import PhyNetPy.GraphUtils as utils
...
"""
seed = np.random.randint(0, 1000) #240
#print(seed)
...
def _nodes_to_improve(net : Network, 
    ...
    if net.out_degree(n) == 0:
        nti[n] = lti[n]
        # if nti[n] > 0:
        #     print(f"Leaf {n.label} needs {nti[n]} ploidy")
    else: #recursive case
        child_needs = [_nodes_to_improve(net, node, nti, lti) for node in net.get_children(n)]
        nti[n] = min(child_needs)
        # if nti[n] > 0:
        #     print(f"Internal node {n.label} needs {nti[n]} ploidy (children: {child_needs})")
    return nti[n]
...
def _attach(net : Network, nti : dict[Node, int], goal_ploidy : dict[Node, int]) -> None:
    """
    ...
    """
    #utils.printable_dict(nti)
    
    #Get all clusters, including clusters of size 1
    clusters : set[tuple[Node]] = utils.get_all_clusters(net, 
                                                         include_trivial = True)
    
    #Sort clusters by size, large to small
    sorted_clusters = sorted(clusters, key=len, reverse=True)
    #print([cluster_as_name_set(cluster) for cluster in sorted_clusters])
    
    #Can only increase the cluster by the minimum difference in ploidy across its leaves
    def amt_allowed(cluster : tuple[Node], lti) -> int:
        #utils.printable_cluster(cluster)
        return min([lti[node] for node in cluster])
    
    #Select the largest cluster whose leaves all need upgrading
    chosen_cluster = None
    ploidy_amt = 0
    index = 0
    while index <= len(sorted_clusters):
        candidate = sorted_clusters[index]
    ...
    if chosen_cluster is None:
        raise Exception("there were no clusters whose taxa needed more ploidy")
    
    top_of_cluster : Node = net.mrca(set(chosen_cluster))
    #print(top_of_cluster.label)
    valid_edges = net.subgenome_ct_edges(downstream_node = top_of_cluster,
                                         delta = max_ploidy)
    
    src : Edge = random_object(list(valid_edges.keys()), RNG)
    #print(src.src.label, src.dest.label)
    if top_of_cluster != net.root():
        add_hybrid(net, src, net.in_edges(top_of_cluster)[0])
    else:
        raise NetworkError("Uh oopsy")
...
def _resolve_ploidy(net : Network, 
    ...
    while not _ploidy_dif(goal_ploidy, cur_ploidy) and iteration < max_iterations:
        ...
        # Only consider leaves that haven't reached their target yet
        lti : dict[Node, int]= {}
        for node in net.get_leaves():
            needed = goal_ploidy[node] - cur_ploidy[node]
            lti[node] = needed
        
        #Determine valid reticulation spots and add the hybrid edge
        #old_ploidy = cur_ploidy.copy()
...
def allele_map_set_ilp(g : Network, gene_map : dict[str, list[str]], max_solutions : int = 10) -> list[AlleleMap]:
    """
    Compute valid allele maps using Integer Linear Programming.
    Finds multiple solutions by iteratively adding constraints to exclude
    previously found solutions.
    
    Args:
        g (Network): A gene tree
        gene_map (dict[str, list[str]]): A taxon map.
        max_solutions (int): Maximum number of solutions to find. Defaults to 10.

    Returns:
        list[AlleleMap]: a list of functions that map labels of g to 
                         labels of a MUL tree.
    """
    # Get all gene tree leaves and their possible MUL tree leaf mappings
    gene_leaves = g.get_leaves()
    mul_leaves = set()
    for copies in gene_map.values():
        mul_leaves.update(copies)
    
    # Create binary variables for each possible mapping
    # x[i,j] = 1 if gene leaf i maps to MUL leaf j
    ...
    # Keep finding solutions until we hit max_solutions or no more solutions exist
    while len(solutions) < max_solutions:
        # Create a new problem for each iteration
        prob = LpProblem("Allele_Mapping", LpMaximize)
        
        # Objective: Maximize number of valid mappings
        prob += lpSum(x[i.label, j] for i in gene_leaves for j in mul_leaves)
        
        # Constraints:
        # 1. Each gene leaf must map to exactly one MUL leaf
        for i in gene_leaves:
            prob += lpSum(x[i.label, j] for j in mul_leaves) == 1
        
        # 2. Each MUL leaf can be mapped to at most once
        for j in mul_leaves:
            prob += lpSum(x[i.label, j] for i in gene_leaves) <= 1
        
        # 3. Gene leaves can only map to valid MUL leaves based on gene_map
        for i in gene_leaves:
        ...
        # 4. Add constraints to exclude previously found solutions
        for prev_solution in solutions:
            # For each previous solution, add a constraint that at least one
        ...
        # Convert solution to AlleleMap object
