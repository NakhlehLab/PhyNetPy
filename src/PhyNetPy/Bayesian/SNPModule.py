...
def print_csdict(F, site_count):

    for site in range(site_count):
        for nr_key in F[site].Keys:
            # n = nr_key.Item1.NX[0]
            # r = nr_key.Item1.RX[0]
            prob = F[site][nr_key]
            
            print(f"NX = {[num for num in nr_key.Item1.NX]}")
            print(f"RX = {[num for num in nr_key.Item1.RX]}")
            print(f"Prob: {prob}")
    ...
    def reorder_vpi(self, vpi_key: tuple, site_count:int, branch_index:int, for_top : bool) -> tuple:
        ...
        for site in range(site_count):
            for tup in F_map[site].Keys:
                tup.Item1.MoveToEnd(former_index)
