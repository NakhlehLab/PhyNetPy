...
class SNAPPLikelihood:
    

    def __init__(self):
        self.ALGORITHM = 0
        self.useOnlyPolymorphic = False
        self.usePseudoLikelihood = False
        self.useApproximateBayesian = False
        self.timeSavingMode = False
        self.debugMode = False 
        self.workloadCounter = None #LongAdder class
        self.pseudoLikelihood = None #SNAPPPseudoLikelihood class
        self.approximateBayesian = None #ApproximateBayesian class

    def getModel(self, markerSeqs):  # BiAllelicGTR return type
        pi =  []
        rate = []
        rate[0] = 0.0
        pi[0] = 0.0
        ...
        count = [0,0,0]
        for alg in markerSeqs: #TODO: MarkerSeq class
            for s in alg.getAlignment().values():
        ...
        if(MHSettings._DIPLOID_SPECIES == None):
        ...
        BAGTRModel = BiAllelicGTR(pi, rate)

        return BAGTRModel
    

    def randomlyPhasing(self, sequence, random): #Map from strings to strings return type
        if(random == None):
            random = random
        newbuilder = {}
        for taxon in sequence.keys():
            newbuilder[taxon] = ""
            for i in range(len(sequence[taxon])):
                c = sequence[taxon][i]
                if(c == '1'):
    ...
    def polyploidSequenceToPatterns(self, alleles2species, markerSeqs, totalPerSpecies): #Map<RPattern, double[]>  return type
        result = {}
        maxLineages = {}
        for aln in markerSeqs:
            for i in range(aln.getSiteCount()):
                currentPattern = {} #item1[0]: n, item2[0]: R0
                weight = 1.0
                for allele in aln.getTaxaNames():
                    c = aln.getAlignment().get(allele)[i]
                    species = ""
                    if(alleles2species == None):
                        species = allele
                    else:
                        species = alleles2species[allele]

                    if(not currentPattern.containsKey(species)): ##contains key in python
                        currentPattern[species] = [[0],[0]]
                    if (c == '-' or c == '?'):
                        continue
                    else:
                        currentPattern[species][1][0] += totalPerSpecies - (c - '0')
                    
                    currentPattern[species][0][0] += totalPerSpecies
                ...
                notGood = False
                newPattern = {}
                for species in currentPattern.keys():
                    if(currentPattern[species][0][0] == 0):
                        notGood = True   #ignore the case that there is a branch contains no data
                    weight /= ArithmeticUtils.binomialCoefficient(currentPattern[species][0][0], currentPattern[species][1][0])
                    newPattern[species] = R(currentPattern[species][0][0], currentPattern[species][1])
                

                if(notGood):
                        continue

                for species in newPattern.keys():
                    if(not maxLineages.containsKey(species)): #containsKey
                        maxLineages[species] = 0
                    maxLineages[species] = math.max(maxLineages[species], newPattern[species].getN())
        

                rpattern = RPattern(newPattern)
                if(self.useOnlyPolymorphic and rpattern.isMonomorphic()):
                        continue

                if(not result.containsKey(rpattern)): ##contains key
                        result[rpattern] =[0.0, 0.0]
                result[rpattern][0] += 1.0
                result[rpattern][1] += math.log(weight)
          

        if(self.useOnlyPolymorphic):
            for i in range(R.dims):
                newPattern = {}

                for species in maxLineages.keys():
                    a= [int(x) for x in R.Dims]
                    if(i < R.dims):
                        a[i] = maxLineages[species]
                    newPattern[species] = R(maxLineages[species], a)
    ...
    def diploidSequenceToPatterns(self, alleles2species, markerSeqs): #Map<RPattern, double[]> return type
        result = {}
        maxLineages = {}
        for aln in markerSeqs:
            for i in range(aln.getSiteCount()):
                ...
                weight = 1.0
                for allele in aln.getTaxaNames():
                    c = aln.getAlignment().get(allele).charAt(i)
                    species = ""
                    if(alleles2species == None):
                        species = allele
                    else:
                        species = alleles2species.get(allele)
                    if(not currentPattern.containsKey(species)):
                        currentPattern[species]= [[0],[0]]
                    if(c == '-' or c == '?'):
                        continue
                    elif(c == '0'):
                        currentPattern[species][1][0] += 2
                    elif(c == '1'):
                        currentPattern[species][1][0] += 1
                    currentPattern[species][0][0] += 2
                ...
                for species in newPattern.keys():
                    if(not maxLineages.containsKey(species)):
                        maxLineages[species]=0
                    maxLineages[species] = math.max(maxLineages[species], newPattern[species].getN())
                ...
                if(not result.containsKey(rpattern)):
                    result[rpattern] = [0.0, 0.0]
                result[rpattern][0] += 1.0
                result[rpattern][1] += math.log(weight)
        ...
        if(self.useOnlyPolymorphic):
            for i in range(R.dims + 1):
                newPattern = {}

                for species in maxLineages.keys():
                    a = [int(x) for x in R.dims]
                    if(i < R.dims): ##indentation messed up, not sure if next two lines in if or not
                        a[i] = maxLineages[species]
                        newPattern[species] = R(maxLineages[species], a)
        ...
        for(MarkerSeq aln : markerSeqs) {
            for(int i = 0  i < aln.getSiteCount()  i++) {
                ...
                for(String species : currentPattern.keySet()) {
                    ...
                    weight /= ArithmeticMHSettings.binomialCoefficient(currentPattern.get(species).Item1[0], currentPattern.get(species).Item2[0])
    ...
    static public double computeApproximateBayesian(Network network, Map<String, String> alleles2species, List<MarkerSeq> markerSeqs, BiAllelicGTR BAGTRModel, Map<String, Double> abcData) {
        if(approximateBayesian == None) {
            System.out.println("Initiating approximate bayesian")
            approximateBayesian = new ApproximateBayesian(alleles2species, markerSeqs, markerSeqs.get(0)._diploid, markerSeqs.get(0)._dominant != None, markerSeqs.get(0)._polyploid, useOnlyPolymorphic, BAGTRModel)
    ...
    static public double computeSNAPPLikelihoodST(Network network, Map<RPattern, double[]> patterns, BiAllelicGTR BAGTRModel) {
        ...
        if(!MHSettings._ESTIMATE_POP_SIZE) {
            net.getRoot().setRootPopSize(MHSettings._POP_SIZE_MEAN)
        }

        Double theta = None
        if(MHSettings._CONST_POP_SIZE) {
    ...
    static public double computeSNAPPLikelihoodMTC(Network network, Map<RPattern, double[]> patterns, BiAllelicGTR BAGTRModel) {
        ...
        ExecutorService executor = Executors.newFixedThreadPool(MHSettings._NUM_THREADS)
        ...
        # /*if(MHSettings._TAXON_MAP != None) {
        #     maxLineages = 0
        #     for (String species : MHSettings._TAXON_MAP.keySet()) {
        #         for (String allele : MHSettings._TAXON_MAP.get(species)) {
        #             maxLineages++
        #         }
        #     }
        ...
        for(int i = 0  i < jobLists.size()  i++) {
            ...
            executor.execute(new Runnable() {
                public void run() {
                    ...
                    Network cloneNetwork = None
                    SNAPPAlgorithm run = None
                    ...
                    for (int j = 1 j < jobList.size() j++) {
                        ...
                        likelihood = run.updateProbability(0, nextJob.Item1, None, node2update)
